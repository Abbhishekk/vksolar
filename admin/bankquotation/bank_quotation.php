...
if ($bankQuery) {
    while ($row = $bankQuery->fetch_assoc()) {
        $banks[] = $row;
    }
}
   ...
   <?php require_once __DIR__ . '/../include/head2.php'; ?>
    ...
    <style>
        ...
        * {
        ...
        .form-group.span-2 {
    grid-column: 1 / -1;   /* safest with auto-fit */
}
...
?>
    ...
    <script>
        ...
        function generateQuotation() {
            ...
            let isValid = true;
            const requiredFields = ['customer_name', 'customer_address', 'plant_capacity', 'system_description', 'quotation_number','estimated_generation','bank_id'];
...
/* ======================================================
   SAVE QUOTATION (MASTER + PRODUCTS)
====================================================== */
            fetch('save', {
    method: 'POST',
    headers: {
    ...
    body: JSON.stringify({
        client_id: <?= json_encode($_SESSION['bank_quotation_client_id']); ?>,
        ...
        products: products
            ...
            .then(resp => {
                if (!resp.status) {
                    ...
                    alert('Failed to save quotation: ' + resp.message);
                }
            })
            .catch(err => {
                console.error('Save error:', err);
                alert('Unexpected error while saving quotation');
            });
                        ...
                        <div class="terms-section-preview">
                            ...
                            <div class="terms-column-preview">
                                ...
                                <div class="bank-details-preview">
                                    <div><strong>Bank Name:</strong> ${SELECTED_BANK.bank_name || ''}</div>
                                    <div><strong>Branch Name:</strong> ${SELECTED_BANK.branch_name || ''}</div>
                                    <div><strong>IFSC Code:</strong> ${SELECTED_BANK.ifsc_code || ''}</div>
                                    <div><strong>Account Number:</strong> ${SELECTED_BANK.account_number || ''}</div>
                                    <div><strong>Account Type:</strong> ${SELECTED_BANK.account_type || ''}</div>
                                    <div><strong>GST No:</strong> ${SELECTED_BANK.bank_gst || ''}</div>
                                </div>
...
.header-image-preview {
    ...
    display: block;
        ...
        function downloadPDF() {
            ...
            const element = document.querySelector('#printableQuotation');
   ...
   <script>
/* ======================================================
   PREFILL DATA FROM INDEX.PHP
====================================================== */
document.addEventListener('DOMContentLoaded', function () {

    if (typeof prefillData === 'undefined') {
        console.warn('Prefill data not found');
        return;
    }

    /* ------------------------------
       1. SIMPLE INPUT PREFILL
    ------------------------------ */
    const fieldMap = [
        'customer_name',
        'customer_address',
    ...
    fieldMap.forEach(field => {
        if (prefillData[field] !== undefined && document.getElementById(field)) {
            document.getElementById(field).value = prefillData[field];
        }
    });
    ...
    /* ------------------------------
       2. TOTALS PREFILL
    ------------------------------ */
    if (prefillData.subsidy !== undefined) {
        document.getElementById('subsidy').value = prefillData.subsidy;
    }
...
<script>
/* ======================================================
   SAFE PRODUCT PREFILL (ONE BY ONE)
====================================================== */
document.addEventListener('DOMContentLoaded', function () {

    if (!prefillData || !Array.isArray(prefillData.products)) {
        return; // normal flow
    }

    const products = prefillData.products;

    if (products.length === 0) {
        return; // keep default product untouched
    }

    // ---- FILL FIRST PRODUCT (ALWAYS EXISTS IN HTML) ----
    const first = products[0];

    document.querySelector('.product-description').value = first.description || '';
    document.querySelector('.product-quantity').value    = first.quantity || 1;
    document.querySelector('.product-unit-price').value  = first.unitPrice || 0;

    // ---- ADD EXTRA PRODUCTS IF ANY ----
    for (let i = 1; i < products.length; i++) {
        addProduct();

        const index = productCounter;

        document.querySelector(`#product-${index} .product-description`).value = products[i].description || 'Sale of solar power generating system including supply, installation, and commissioning of 3kW On-Grid solar PV system';
        document.querySelector(`#product-${index} .product-quantity`).value    = products[i].quantity || 1;
        document.querySelector(`#product-${index} .product-unit-price`).value  = products[i].unitPrice || 0;
    }
