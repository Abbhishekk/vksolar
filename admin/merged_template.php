...
// merged_template.php (now DB-based)
session_start();
require_once "connect/db.php"; // $conn from Database class

// 1) Validate quote_id from URL
if (!isset($_GET['quote_id']) || !is_numeric($_GET['quote_id'])) {
    die("Invalid quotation request. Quote ID missing or incorrect.");
}
...
if (!$result || $result->num_rows === 0) {
    die("Quotation not found for the given ID.");
}

$row = $result->fetch_assoc();

// 3) Map DB columns → PHP variables (same naam jo tum template me use kar rahe ho)
...
// System config
$system_size    = $row['system_size_kwp'] ?? '';         // kW(p)
...
// Panel / inverter
$panel_company    = $row['panel_company'] ?? '';
$panel_model      = $row['panel_wattage'] ?? '';         // e.g. 550 (W)
$panel_quantity   = $row['panel_count'] ?? '';
$inverter_company = $row['inverter_company'] ?? '';
$inverter_type    = $row['inverter_type'] ?? '';
$inverter_capacity= $row['inverter_capacity'] ?? '';     // e.g. 5, 10 (kVA)
$inverter_quantity= $row['inverter_count'] ?? '';

// Financials + savings
$current_monthly_bill     = $row['monthly_bill'] ?? 0;
$investment               = $row['total_cost'] ?? 0;     // Total system cost (before subsidy)
$subsidy_amount           = $row['subsidy'] ?? 0;
$final_cost               = $row['final_cost'] ?? 0;

$estimated_monthly_savings = $row['monthly_savings'] ?? 0;
...
if ($estimated_yearly_savings > 0) {
    $payback_period = round($final_cost / $estimated_yearly_savings, 1);
} else {
    $payback_period = 0;
}

// If payback period exists in database, use it, otherwise use calculated value
$payback_period = $row['payback_period'] ?? $payback_period;

// "Vendor / Generated by" mapping → from preparer fields
$vendor_name    = $row['prepared_by'] ?? '';
$vendor_contact = $row['preparer_contact'] ?? '';
$vendor_email   = $row['preparer_email'] ?? '';
$vendor_address = $row['preparer_address'] ?? '';

// Extra flags if you want to show anywhere
$battery_backup     = !empty($row['battery_backup']);
$smart_monitoring   = !empty($row['smart_monitoring']);
$annual_maintenance = !empty($row['annual_maintenance']);

// Optional: quote number & status to print in header
$quote_number = $row['quote_number'] ?? '';
$status       = $row['status'] ?? '';

$feedbackUrl = "feedback?quote_id=" . $quoteId;
?>
            ...
            <div class="details-panel">
                ...
                <div class="details-block">
                    ...
                    <p>
                        ...
                        <strong>Address :</strong> <?php echo htmlspecialchars($address); ?><br>
            ...
            <table class="materials-table">
                ...
                <tbody>
                    <tr>
                        ...
                        <td>24V, <?php echo $panel_model; ?>Wp</td>
                        <td><?php echo $panel_quantity; ?></td>
                        <td><?php echo strtoupper($panel_company); ?></td>
                    </tr>
                    <tr>
                        <td>Solar Grid Inverter</td>
                        <td>1 PH, <?php echo ceil($inverter_capacity); ?> KW</td>
                    ...
                    <tr>
                        ...
                        <td><?php echo number_format($system_size, 1); ?> kW</td>
        ...
        <div class="specifications">
            ...
            <div class="spec-grid">
                <div class="spec-item">
                    ...
                    <span><?php echo number_format($system_size, 2); ?> kWp (<?php echo $panel_quantity; ?> x <?php echo $panel_model; ?>Wp modules)</span>
                </div>
                <div class="spec-item">
...
<!--    <section id="cost-breakdown" class="newcontainer print-keep-together">-->
<!--    <div class="cost-card">-->
<!--        <h2 class="cost-heading">Project Cost Breakdown</h2>-->
<!--        <p class="cost-subtitle">-->
<!--            The following table provides a detailed breakup of the proposed solar rooftop system cost,-->
<!--            including material, installation and statutory charges.-->
<!--        </p>-->

<!--        <div class="cost-table-wrapper">-->
<!--            <table class="cost-table" cellspacing="0" cellpadding="0">-->
<!--                <thead>-->
<!--                    <tr>-->
...
<!--                        <td>High-efficiency Mono PERC panels</td>-->
<!--                        <td>XX kWp</td>-->
<!--                        <td>XX,XXX</td>-->
<!--                        <td>XX,XXX</td>-->
<!--                    </tr>-->
...
<!--                        <td>On-grid / Hybrid inverter with MPPT</td>-->
<!--                        <td>1 No.</td>-->
<!--                        <td>XX,XXX</td>-->
<!--                        <td>XX,XXX</td>-->
<!--                    </tr>-->
...
<!--                        <td>Hot-dip galvanized / Aluminium structure</td>-->
<!--                        <td>Complete</td>-->
<!--                        <td>XX,XXX</td>-->
<!--                        <td>XX,XXX</td>-->
<!--                    </tr>-->
...
<!--                        <td>4</td>-->
<!--                        <td>BOS & Cabling</td>-->
<!--                        <td>AC/DC cables, junction boxes, protections</td>-->
<!--                        <td>LS</td>-->
<!--                        <td>XX,XXX</td>-->
<!--                        <td>XX,XXX</td>-->
<!--                    </tr>-->
...
<!--                        <td>Labour, testing & documentation</td>-->
<!--                        <td>LS</td>-->
<!--                        <td>XX,XXX</td>-->
<!--                        <td>XX,XXX</td>-->
<!--                    </tr>-->
<!--                    <tr>-->
<!--                        <td>6</td>-->
<!--                        <td>Net Metering & Statutory Charges*</td>-->
<!--                        <td>DISCOM & government related fees (if applicable)</td>-->
<!--                        <td>LS</td>-->
<!--                        <td>XX,XXX</td>-->
<!--                        <td>XX,XXX</td>-->
<!--                    </tr>-->
...
<!--                    </tr>-->
<!--                    <tr>-->
<!--                        <td colspan="5" class="text-right total-label">Total Project Cost (₹)</td>-->
<!--                        <td class="total-value">XX,XXX</td>-->
<!--                    </tr>-->
<!--                </tfoot>-->
<!--            </table>-->
<!--        </div>-->
...
<!--        <p class="cost-note">-->
<!--            *Net metering and statutory charges are indicative and subject to actual DISCOM / government norms-->
    ...
    <div class="roi-card">
        ...
        <div class="roi-summary">
            ...
            <div class="roi-badge">
                ...
                <span class="value">₹ <?php echo number_format($final_cost, 0); ?></span>
            </div>
            <div class="roi-badge">
        ...
        <div class="roi-table-wrapper">
            <table class="roi-table" cellspacing="0" cellpadding="0">
                ...
                <tbody>
                    <?php
                    $degradation_rate = 0.02; // 2% degradation per year
                    $units_per_kwp = 1460; // units per kWp per year
                    $years = [1, 2, 3, 4, 10, 25];
                    $payback_row_done = false;
                    
                    foreach ($years as $year) {
                        $degradation_factor = pow(1 - $degradation_rate, $year-1);
                        $year_generation = $system_size * $units_per_kwp * $degradation_factor;
                        $year_savings = $estimated_yearly_savings * $degradation_factor;
                        $cumulative_savings = $estimated_yearly_savings * (1 - pow(1 - $degradation_rate, $year)) / $degradation_rate;

                        $is_payback_row = false;
                        if (!$payback_row_done && $cumulative_savings >= $final_cost) {
                            $is_payback_row = true;
                            $payback_row_done = true;
                        }

                        echo '<tr'.($is_payback_row ? ' class="roi-payback-row"' : '').'>';
                        echo '<td>'.$year.'</td>';
                        echo '<td>'.number_format($year_generation, 0).'</td>';
                        echo '<td>₹ '.number_format($year_savings, 0).'</td>';
                        echo '<td>₹ '.number_format($cumulative_savings, 0).'</td>';
                        if ($is_payback_row) {
                            echo '<td><strong>Approx. Payback Achieved</strong></td>';
                        } else {
                            echo '<td>&nbsp;</td>';
                        }
                        echo '</tr>';
    ...
    <div class="partner-card">
        ...
        <div class="partner-cols">
            ...
            <div class="partner-box">
                ...
                <p><span class="label">Prepared By:</span> <span><?php echo htmlspecialchars($vendor_name ?: 'Not specified'); ?></span></p>
                <p><span class="label">Preparer Contact:</span> <span><?php echo htmlspecialchars($vendor_contact ?: 'Not specified'); ?></span></p>
                <p><span class="label">Preparer Email:</span> <span><?php echo htmlspecialchars($vendor_email ?: 'Not specified'); ?></span></p>
            </div>
        </div>
    </div>
    <div class="partner-card">
        ...
        <div class="response-buttons">
            <a href="<?php echo htmlspecialchars("feedback?quote_id=" . $quoteId); ?>" class="response-btn review">
                ⏳ FEEDBACK
            </a>
