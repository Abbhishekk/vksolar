...
?>
                    ...
                    <form id="declarationForm">
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-grid">
                                ...
                                <div class="form-group">
                                    <label for="inverter_no" class="required">Inverter No</label>
                                    <input type="text" class="form-control" id="inverter_no" name="inverter_no" required>
                                </div>
                                ...
                                <div class="form-group">
                                    <label for="pannel_serial" class="required">Sr No Of PV Module</label>
                                    <textarea class="form-control" id="pannel_serial" name="pannel_serial" rows="2" required></textarea>
                                </div>
                        ...
                        <div class="form-section">
                            ...
                            <div class="form-grid">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="declarant_name" name="declarant_name" required readonly>
                                </div>
                                <div class="form-group">
                                    <label for="declarant_designation" class="required">Declarant Designation</label>
                                    <input type="text" class="form-control" id="declarant_designation" name="declarant_designation" required readonly>
                                </div>
                                <div class="form-group">
                                    <label for="declarant_email" class="required">Email Address</label>
                                    <input type="email" class="form-control" id="declarant_email" name="declarant_email" required readonly>
                                </div>
                                <div class="form-group">
    ...
    if (
        ...
        !formData.get('system_capacity') ||
        !formData.get('consumer_name') ||
        !formData.get('project_address')
    ) {
        alert('Please fill in all required fields');
        return;
    }
    ...
    .then(response => {
        ...
        const declarationHTML = createDeclarationHTML(
            data,
                ...
                <div class="pdf-optimized">
                    ...
                    <div class="declaration-content">
                        <div class="declaration-section">
                            ...
                            <div class="pv-module-details class="mb-0"">
                                <div class="pv-module-item">
                                    <div class="pv-module-label">1.PV Module Capacity:</div>
                                    <div class="pv-module-value">${data.pv_module_capacity} W</div>
                                </div>
                                <div class="pv-module-item">
                                    <div class="pv-module-label">2.Number of PV Modules:</div>
                                    <div class="pv-module-value">${data.pv_module_count}</div>
                                </div>
                                <div class="pv-module-item">
                                    <div class="pv-module-label">3.Invertor NO:</div>
                                    <div class="pv-module-value">${data.inverter_no}</div>
                                </div>
                                <div class="pv-module-item">
                                    <div class="pv-module-label">4.Sr NO Of PV Module:</div>
                                    <div class="pv-module-value">
                                    <?php
                                        $serials = $client['panel_serial_numbers'] ?? '';
                                        
                                        if (!empty($serials)) {
                                            $serialArray = array_map('trim', explode(',', $serials));
                                        
                                            echo "<ul style='display:flex; flex-wrap:wrap; gap:10px 20px; list-style:none; padding-left:0; margin:0;'>";
                                ...
                                <div class="pv-module-item">
                                    <div class="pv-module-label">5.PV Module Make:</div>
                                    <div class="pv-module-value">${data.pv_module_make}</div>
                                </div>
                                <div class="pv-module-item">
                                    <div class="pv-module-label">6.Cell manufacturer's name:</div>
                                    <div class="pv-module-value">${data.cell_manufacturer}</div>
                                </div>
                                <div class="pv-module-item">
                                    <div class="pv-module-label">7.Cell GST invoice No:</div>
                                    <div class="pv-module-value">${data.cell_gst_invoice}</div>
                                </div>
                            </div>
                            <p class="mt-0">3. The above undertaking is based on the certificate issued by PV Module manufacturer/supplier while supplying the above mentioned order.</p>

                            <p>4. I, <span class="highlight">Vilas Kale </span> on behalf of M/S <span class="highlight">VK SOLAR ENERGY</span> further declare that the information given above is true and correct and nothing has been concealed therein. If anything is found incorrect at any stage, then REC/ MNRE may take any appropriate action against my company for wrong declaration. Supporting documents and proof of the above information will be provided as and when requested by MNRE.</p>
                        </div>
                        <div style="display:flex; justify-content:right;">
                        <div class="signature-area">
                            <p>(Signature With official Seal)</p>
                            <div>
                                <img src="https://vksolarenergy.com/admin/documentmanagement/img/stamp.png" style="width: 60%; height : 50px; object-fit: contain;" >
                            </div>
                            <div style="margin-top: 10px;">
                                <p style="margin-top: 2px;margin-bottom: 2px;">For M/S <span class="highlight" >VK SOLAR ENERGY</span></p>
                                <p style="margin-top: 2px;margin-bottom: 2px;">Name: <span class="highlight">MR VILAS KALE</span></p>
                                <p style="margin-top: 2px;margin-bottom: 2px;">Designation: <span class="highlight">PROPRIETOR</span></p>
                                <p style="margin-top: 2px;margin-bottom: 2px;">Phone: <span class="highlight">${data.declarant_phone}</span></p>
                                <p style="margin-top: 2px;margin-bottom: 2px;">Email: <span class="highlight">vksolarenergy1989@gmail.com</span></p>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const client = {
        ...
        system_capacity: "<?= htmlspecialchars($prefill['system_capacity'] ?? '') ?>",
        consumer_name: "<?= htmlspecialchars($prefill['consumer_name'] ?? '') ?>",
        project_address: "<?= htmlspecialchars($prefill['project_address'] ?? '') ?>",
        inverter_no: "<?= htmlspecialchars($prefill['inverter_no'] ?? '') ?>",
        application_number: "<?= htmlspecialchars($prefill['application_number'] ?? '') ?>",
        pv_module_capacity: "<?= htmlspecialchars($prefill['pv_module_capacity'] ?? '') ?>",
        pv_module_count: "<?= htmlspecialchars($prefill['pv_module_count'] ?? '') ?>",
        pannel_serial: "<?= htmlspecialchars($prefill['pannel_serial'] ?? '') ?>",
        pv_module_make: "<?= htmlspecialchars($prefill['pv_module_make'] ?? '') ?>",
        discom_name: "<?= htmlspecialchars($prefill['discom_name'] ?? '') ?>",
        declarant_name: "MR VILAS KALE",
        declarant_designation:"PROPRIETOR",
        declarant_phone: "",
        declarant_email: "vksolarenergy1989@gmail.com"
    };
