...
function generateCertificate() {
    ...
    /* ---------- SAVE FIRST (LIKE WCR) ---------- */
    fetch("save_undertaking", {
        method: "POST",
        body: new FormData(document.getElementById('undertakingForm'))
    });
                    ...
                    <div class="stamp-container">
                        <div class="signature-block">
                            <div class="stamp-block">
                                ...
                                <div>
                                    <img src="https://vksolarenergy.com/admin/documentmanagement/img/stamp.png" style="width: 30%; height : 70px; object-fit: contain;" >
                                </div>
                            </div>
                            <p><strong>Name of Agent/Representative:</strong></p>
                            <p><strong>Mr.VILAS KALE</strong></p>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const client = {
        ...
        agent_name: "MR VILAS KALE",

        consumer_name: "<?= htmlspecialchars($client['name'] ?? '') ?>",
        consumer_number: "<?= htmlspecialchars($client['consumer_number'] ?? '') ?>",

        consumer_address: "<?= htmlspecialchars(
            trim(
                ($client['location'] ?? '') . ' ' .
                ($client['village'] ?? '') . ' ' .
                ($client['taluka'] ?? '') . ' ' .
        ...
        system_load: "<?= htmlspecialchars($client['kilo_watt'] ?? '') ?>",

        // Optional defaults
        has_backup_inverter: "<?= htmlspecialchars($client['bank_loan_status'] === 'yes' ? 'yes' : 'no') ?>"
    };
